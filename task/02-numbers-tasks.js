...
function getAngleBetweenVectors(x1, y1, x2, y2) {
   return Math.abs(Math.asin(y1/(Math.pow(x1,2)+Math.pow(y1,2)))-Math.asin(y2/(Math.pow(x2,2)+Math.pow(y2,2))));
}
...
function toNumber(value, def) {
    let tmp=value;
    if(value instanceof Object){tmp=value.valueOf();}
    let tmp1=+tmp;
    return (typeof tmp1==='number'&&!Number.isNaN(tmp1))?tmp1:def;
}
