...
function encodeToRot13(str) {
    ...
    for (var i = 0; i < str.length; i++) {
        let tmp1=str.charCodeAt(i);
        let tmp2;
        if(tmp1>=65&&tmp1<=90){newstr+=(tmp1<78)?String.fromCodePoint(tmp1+13):String.fromCodePoint(tmp1-13);}
        else if(tmp1>=97&&tmp1<=122){newstr+=(tmp1<110)?String.fromCodePoint(tmp1+13):String.fromCodePoint(tmp1-13);}
        else {newstr+=String.fromCodePoint(tmp1);}
    }
    return newstr;
}
...
function isString(value) {
    return (value instanceof Object)?(typeof value.valueOf()==='string'):typeof value==='string';
}
